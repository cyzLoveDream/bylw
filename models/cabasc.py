...
class CABASC(BasicModule):
	def __init__(self,embedding_matrix,opt,type='c'):
		super(CABASC,self).__init__()
		self.opt = opt
		self.model_name = "cabasc"
		self.types = type
		...
		self.dense = nn.Linear(opt.embed_dim,opt.polarity_dim)  # W5
		self.weight9 = nn.Parameter(torch.Tensor(opt.embed_dim, opt.embed_dim))
		self.weight8 = nn.Parameter(torch.Tensor(opt.embed_dim, opt.embed_dim))
		
		# context attention layer
		self.rnn_l = DynamicLSTM(opt.embed_dim,opt.hidden_dim,num_layer=1,batch_first=True,rnn_type='GRU')
		self.rnn_r = DynamicLSTM(opt.embed_dim,opt.hidden_dim,num_layer=1,batch_first=True,rnn_type='GRU')
	...
	def forward(self,inputs):
		...
		x = torch.matmul(x, self.weight8) + torch.matmul(v_s, self.weight9)
		# content attention module
		v_ts = x
		for _ in range(self.opt.hops):
			# x = self.x_linear(x)
			v_ts = self.attention(memory, v_ts)
